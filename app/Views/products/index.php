...
<body>
    ...
    <section>
        <h1>Products List</h1>
        <button type="button" class="btn btn-primary" id="btnAddNewProductModal">Add Products</button>
        <button type="button" class="btn btn-success" id="btnFinalSubmit">Submit</button>
        <table class="table">
            <thead>
                <tr>
                    <th scope="col">#</th>
        ...
        <div class="modal fade" id="addNewProductModal" tabindex="-1" role="dialog" aria-labelledby="addNewProductModal" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form name="addProductForm" method="post" id="addProductForm" action="<?=site_url('products/create')?>" enctype="multipart/form-data">
                        <div class="modal-body">  
                            <div class="form-group response-container" style="display: none;">
                                
                            </div>                          
                            <div class="form-group mb-2">
                                <label for="exampleInputEmail1">Title</label>
                                <input type="text" class="form-control product_title" name="product_title" placeholder="Enter title" pattern="^[a-zA-Z0-9 ]+$" required>
                                <small class="product_title_error error-element"></small>
                            </div>
                            <div class="form-group mb-2">
                                <label for="exampleInputPassword1">Price</label>
                                <input type="text" class="form-control product_price" name="product_price" placeholder="Price" required>
                                <small class="product_price_error error-element"></small>
                            </div>
                            <div class="form-group mb-2">
                                <label for="exampleInputPassword1">Image</label>
                                <input type="file" class="form-control product_image" name="product_image" accept="image/png, image/jpeg" required>
                                <small class="product_image_error error-element"></small>
                            </div> 
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-primary btnSaveProduct">Save Product</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="modal fade" id="updateProductModal" tabindex="-1" role="dialog" aria-labelledby="updateProductModal" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form name="frmUpdateProduct" method="post" id="frmUpdateProduct">
                        <div class="modal-body">  
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-primary btnUpdateProduct">Update Product</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <script {csp-script-nonce}>
        $(document).ready(function() {
            ...
            $('#btnAddNewProductModal').on('click', function(e) {
            ...
            $('body').on('click', '#btnFinalSubmit', function(e) {
                e.preventDefault();
                saveAllProducts();
            });
        });
        ...
        function updateProduct() {
            let formData = new FormData($('#frmUpdateProduct')[0]);
            $.ajax({
                url: '/products/update',
                method: 'POST',
                data: formData,
                ...
                contentType: false,
                success: function(data) {
                    handleProductResponse(data, '#frmUpdateProduct');
                },
                error: function(xhr, status, error) {
                    // Handle error
                }
        ...
        function saveAllProducts() {
            if (!confirm('Are you sure you want to save these products to the database?')) {
                return false;
            }
            $.ajax({
                url: '/products/save-all',
                method: 'GET',
                dataType: 'json',
                success: function(data) {
                    alert(data.message);
        ...
        function handleProductResponse(data, formSelector = '#addProductForm') {
            if (data.status === true) {
                let products = data.products;
                let productTable = '';
                getProducts();
                $('input').removeClass('validation-error');
                $('small.error-element').html('');
                $('#tblProducts').html(productTable);
                $('.response-container').html(data.message).show();
                ...
                let obj = data.errors;
                let result = Object.keys(data.errors).map((key) => [key, obj[key]]);
                $('input').removeClass('validation-error');
                $('small.error-element').html('');
                result.forEach(function(error) {
                    $('.' + error[0] + '_error').html(error[1]);
                    $('input.' + error[0]).addClass('validation-error');
                });
            }
        }

        function fillUpdateProductForm(product) {
            $('#frmUpdateProduct input.product_id').val(product.product_id);
            $('#frmUpdateProduct input.product_title').val(product.product_title);
            $('#frmUpdateProduct input.product_price').val(product.product_price);
                ...
                success: function(data) {
                    if (data.status === true) {
                        ...
                        products.forEach(function(product) {
                            ...
                            productTable += '<td>' + product.product_title + '</td>';
                            productTable += '<td>' + product.product_price + '</td>';
                            productTable += '<td><img src="' + (product.product_image ? product.product_image : '') + '" width="75"></td>';
                            productTable += `<td><button class="btnEditProduct" data-id="${product.product_id}">Edit</button> | <button class="btnDeleteProduct" data-id="${product.product_id}">Delete</button></td>`;
                            productTable += '</td>';
                        });
                        $('#tblProducts').html(productTable);
                    }
